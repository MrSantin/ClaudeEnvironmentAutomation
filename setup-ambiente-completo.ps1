    ...
    if os.name == "nt":                       # Windows: painel WT com captura via PTY
        ...
        if under_wt:
            ...
            wrapper_src = (
                ...
                "DONE = %r\n"
                "try:\n"
                "    _cols, _rows = shutil.get_terminal_size((120, 30))\n"
                "except Exception:\n"
                "    _cols, _rows = 120, 30\n"
                "try:\n"
                "    _i = CMD.index('-p')\n"
                "    _prompt = CMD[_i + 1] if _i + 1 < len(CMD) else ''\n"
                "    _head = CMD[:_i + 1]\n"
                "except ValueError:\n"
                "    _prompt = ''\n"
                "    _head = CMD\n"
                "sys.stdout.write('=' * _cols + '\\n')\n"
                "sys.stdout.write('COMANDO: ' + ' '.join(_head) + '\\n')\n"
                "sys.stdout.write('-' * _cols + '\\n')\n"
                "sys.stdout.write('PROMPT ENVIADO AO AGY:\\n\\n')\n"
                "sys.stdout.write(_prompt + '\\n')\n"
                "sys.stdout.write('=' * _cols + '\\n')\n"
                "sys.stdout.write('SAIDA DO AGY (tempo real):\\n\\n')\n"
                "sys.stdout.flush()\n"
                "proc = PtyProcess.spawn(CMD, dimensions=(_rows, _cols))\n"
